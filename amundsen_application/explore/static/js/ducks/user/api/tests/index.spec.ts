...
describe('getUser', () => {
  ...
  it('calls axios with correct parameters', async () => {
    ...
    await API.getUser(testId).then(user => {
      expect(axiosMock).toHaveBeenCalledWith(`/explore/api/metadata/v0/user?user_id=${testId}`);
    });
  });
...
describe('getUserOwn', () => {
  ...
  it('calls axios with correct parameters', async () => {
    ...
    await API.getUserOwn(testId).then(data => {
      expect(axiosMock).toHaveBeenCalledWith(`/explore/api/metadata/v0/user/own?user_id=${testId}`);
    });
  });
...
describe('getUserRead', () => {
  ...
  it('calls axios with correct parameters', async () => {
    ...
    await API.getUserRead(testId).then(data => {
      expect(axiosMock).toHaveBeenCalledWith(`/explore/api/metadata/v0/user/read?user_id=${testId}`);
    });
  });
