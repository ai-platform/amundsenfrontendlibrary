...
export function getLoggedInUser() {
  return axios.get(`/explore/api/auth_user`)
...
export function getUser(userId: string, index?: number, source?: string) {
  ...
  return axios.get(`/explore/api/metadata/v0/user?${queryParams}`)
...
export function getUserOwn(userId: string) {
  return axios.get(`/explore/api/metadata/v0/user/own?user_id=${userId}`)
...
export function getUserRead(userId: string) {
  return axios.get(`/explore/api/metadata/v0/user/read?user_id=${userId}`)
