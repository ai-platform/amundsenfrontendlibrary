...
describe('HomePage', () => {
  const setup = (propOverrides?: Partial<HomePageProps>) => {
    const mockLocation = {
      search: '/explore/search?searchTerm=testName&resource=table&pageIndex=1',
    };
  ...
  describe('render', () => {
    ...
    it('contains a Breadcrumb that directs to the /explore/search', () => {
      const element = wrapper.find(Breadcrumb);
      expect(element.exists()).toBe(true);
      expect(element.props().path).toEqual('/explore/search');
    });
